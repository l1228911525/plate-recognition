...
void annrecognise::LoadModel()
{
	ann.clear();
	ann.load(path.c_str(), "ann");
}

void annrecognise::LoadModel(string s)
{
	ann.clear();
...
Mat annrecognise::ProjectedHistogram(Mat img, int t)
{
...
//! 获得字符的特征图
Mat annrecognise::features(Mat in, int sizeData)
{
	//Histogram features
...
//! 利用神经网络做识别
int annrecognise::classify(Mat f, bool isChinses){
	int result = -1;
...
string annrecognise::charsIdentify(Mat input, bool isChinese)
{
	Mat f = features(input, predictSize);
	string result = "";
...
char annrecognise::classifyNumberPlusCharacterText(Mat src)
{
   ...
   /// imshow("ddd", src);
    ...
    Mat feature = features(src, 10);
    ///resize(src, src, Size(5, 5));
    float i = ann.predict(feature, output);
    float maxVal = -2;
    int result = -1;
    for(int j = 0; j < 33; j++)
    {
